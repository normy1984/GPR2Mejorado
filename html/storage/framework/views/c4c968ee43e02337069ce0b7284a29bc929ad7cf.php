...
<?php $component->withAttributes(['id' => $getId(),'label' => $getLabel(),'label-sr-only' => $isLabelHidden(),'helper-text' => $getHelperText(),'hint' => $getHint(),'hint-action' => $getHintAction(),'hint-color' => $getHintColor(),'hint-icon' => $getHintIcon(),'required' => $isRequired(),'state-path' => $getStatePath()]); ?>
    <div
        <?php echo e($attributes
                ...
                ->class(['filament-forms-text-input-component group flex items-center space-x-2 rtl:space-x-reverse']), false); ?>

    >
        <?php if(($prefixAction = $getPrefixAction()) && (! $prefixAction->isHidden())): ?>
            <?php echo e($prefixAction, false); ?>
...
<?php endif; ?>
        ...
        <div class="flex-1">
            <input
                <?php if (! ($hasMask())): ?>
                    x-data="{}"
                    <?php echo e($applyStateBindingModifiers('wire:model'), false); ?>="<?php echo e($getStatePath(), false); ?>"
                    type="<?php echo e($getType(), false); ?>"
                <?php else: ?>
                    x-data="textInputFormComponent({
                        <?php echo e($hasMask() ? "getMaskOptionsUsing: (IMask) => ({$getJsonMaskConfiguration()})," : null, false); ?>

                        state: $wire.<?php echo e($applyStateBindingModifiers('entangle(\'' . $getStatePath() . '\')', lazilyEntangledModifiers: ['defer']), false); ?>,
                    })"
                    type="text"
                    wire:ignore
                ...
                <?php endif; ?>
                dusk="filament.forms.<?php echo e($getStatePath(), false); ?>"
                ...
                <?php endif; ?>
                <?php echo e($getExtraAlpineAttributeBag(), false); ?>
...
<?php endif; ?>
        ...
        <?php if(($suffixAction = $getSuffixAction()) && (! $suffixAction->isHidden())): ?>
            <?php echo e($suffixAction, false); ?>

        <?php endif; ?>
    </div>

    <?php if($datalistOptions): ?>
        <datalist id="<?php echo e($getId(), false); ?>-list">
            <?php $__currentLoopData = $datalistOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $option): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($option, false); ?>" />
