<table class="report-table pdf-table">
    <thead>
    <tr>
        <th colspan="42" style="text-align: center; background-color: #B3B3B3; font-weight: bold; font-size: 18px; height: 30px">{{ trans('reports.poa.title_tracking') }}</th>
    </tr>
    <tr>
        <th colspan="40"></th>
    </tr>
    <tr>
        ...
        <th rowspan="2" style="width: 60px; text-align: center; background-color: #6463b9; color: #FFFFFF">{{ trans('reports.poa.budget_item') }}</th>
		<th rowspan="2" style="text-align: center; background-color: #6463b9; color: #FFFFFF; width: 12px">{{ trans('reports.poa.assigned') }}</th>
        <th rowspan="2" style="text-align: center; background-color: #6463b9; color: #FFFFFF; width: 12px">{{ trans('reports.poa.encoded') }}</th>
        <th rowspan="2" style="text-align: center; background-color: #6463b9; color: #FFFFFF; width: 12px">{{ trans('reports.poa.for_compromising') }}</th>
        <th rowspan="2" style="text-align: center; background-color: #6463b9; color: #FFFFFF; width: 12px">{{ trans('reports.poa.total_accrued') }}</th>
        <!--<th rowspan="2" style="text-align: center; background-color: #6463b9; color: #FFFFFF; width: 12px">{{ trans('reports.poa.encoded') }}</th>
        <th rowspan="2" style="text-align: center; background-color: #6463b9; color: #FFFFFF; width: 12px">{{ trans('reports.poa.total_accrued') }}</th>-->
    ...
    @if(isset($rows))
        @foreach($rows as $row)
            <tr>
                ...
                <td>{{ $row->code }}</td>        
                <td>{{ (float) str_replace(',', '',$row->assigned) }}</td>
                <td>{{ (float) str_replace(',', '', $row->encoded) }}</td>
                <td>{{ (float) str_replace(',', '', $row->for_compromising) }}</td>
                <td>{{ (float) str_replace(',', '',$row->total_amount) }}</td>
            	<td>{{ (float) str_replace(',', '',$row->jan) }}</td>
                <td>{{ (float) str_replace(',', '',$row->feb) }}</td>
                <td>{{ (float) str_replace(',', '',$row->mar) }}</td>
                <td>{{ (float) str_replace(',', '',$row->t1 )}}</td>
                <td>{{ (float) str_replace(',', '',$row->apr) }}</td>
                <td>{{ (float) str_replace(',', '',$row->may) }}</td>
                <td>{{ (float) str_replace(',', '',$row->jun) }}</td>
                <td>{{ (float) str_replace(',', '',$row->t2 )}}</td>
                <td>{{ (float) str_replace(',', '',$row->jul) }}</td>
                <td>{{ (float) str_replace(',', '',$row->aug) }}</td>
                <td>{{ (float) str_replace(',', '',$row->sep) }}</td>
                <td>{{ (float) str_replace(',', '',$row->t3 )}}</td>
                <td>{{ (float) str_replace(',', '',$row->oct) }}</td>
                <td>{{ (float) str_replace(',', '',$row->nov) }}</td>
                <td>{{ (float) str_replace(',', '',$row->dec) }}</td>
                <td>{{ (float) str_replace(',', '',$row->t4 )}}</td>
            </tr>
        @endforeach
    @endif
</table>