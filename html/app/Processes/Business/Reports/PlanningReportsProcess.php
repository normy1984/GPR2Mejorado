...
class PlanningReportsProcess
{
    ...
    public function poaReportIndex()
    {
        ...
        return ['executingUnits' => $executingUnits?? collect()];
    }
    ...
    {
            \Illuminate\Support\Facades\Log::info('Se mostro el bloque de unidad ejecutora',['executingUnitId' => $executingUnitId]);
        $fiscalYear = $this->fiscalYearRepository->findCurrentFiscalYear();//findNextFiscalYear();
        if (!$fiscalYear) {
            throw new Exception(trans('reports.exceptions.fiscal_year_not_found'));
        }
 \Illuminate\Support\Facades\Log::info('Se mostro el bloque de unidad ejecutora',['fiscalYear' => $fiscalYear]);
    ...
    {
        $years = $this->fiscalYearRepository->all();
		Log::info('Respuesta generada years:', ['years' => $years->toArray()]);
        if (!$years->count()) {
            throw  new Exception(trans('reports.exceptions.no_info'), 1000);
        }
